document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelector(
    ".opinions_avg iframe"
  ).src = `/src/components/opinions/opinions.html?id=${localStorage.getItem(
    "id"
  )}`;
  ...
  fetch("http://127.0.0.1:5016/specialist/my-profile/my-information", {
    ...
    .then((user) => {
      ...
      articleName.textContent = `${capitalize(user.name)}, ${capitalize(
        user.last_name
      )}`;

      profileName.textContent = `${capitalize(user.name)}, ${capitalize(
        user.last_name
      )}`;
...
document
  ...
  .addEventListener("click", fetchAndCheckCategories);
